...
import { z } from "zod";
import { ArrowLeft, Plus, Edit2, ShoppingCart, History } from "lucide-react";
...
export function TenantProductDetailPage() {
  ...
  return (
    <div>
      <Button variant="link" onClick={() => navigate({ to: "/tenants/$slug", params: { slug } })} className="mb-4 pl-0">
        ...
        <CardContent className="pt-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center">
              ...
              <p className={`text-2xl font-bold ${totalStock <= product.minStockLevel ? "text-orange-500" : "text-green-500"}`}>{totalStock}</p>
            </div>
            <div className="text-center">
      ...
      <Dialog open={adjustmentModalOpen} onOpenChange={setAdjustmentModalOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Tambah Penyesuaian Stok</DialogTitle></DialogHeader>
          <Form {...adjustmentForm}>
            <form onSubmit={adjustmentForm.handleSubmit((v) => { if (userId) createAdjustmentMutation.mutate({ productId: productId, outletId: v.outletId, quantity: v.quantity, alasan: v.alasan, adjustedBy: userId }); })} className="space-y-4">
              <FormField control={adjustmentForm.control} name="outletId" render={({ field }) => (
                <FormItem><FormLabel>Outlet</FormLabel>
              ...
              <FormField control={adjustmentForm.control} name="quantity" render={({ field }) => (
                <FormItem><FormLabel>Jumlah</FormLabel>
                  <FormControl><Input type="number" placeholder="Masukkan jumlah" {...field} onChange={(e) => field.onChange(Number(e.target.value))} /></FormControl>
                  <p className="text-xs text-gray-500">Angka positif untuk tambah, negatif untuk kurang</p>
                  <FormMessage /></FormItem>
              )} />
              <FormField control={adjustmentForm.control} name="alasan" render={({ field }) => (
                <FormItem><FormLabel>Alasan</FormLabel><FormControl><Textarea placeholder="Masukkan alasan (opsional)" {...field} /></FormControl><FormMessage /></FormItem>
              )} />
