...
export const stockAdjustmentsApi = {
  getAll: async (filters?: { productId?: number; outletId?: number }): Promise<{ data: StockAdjustment[]; meta: any }> => {
    const params = new URLSearchParams();
    if (filters?.productId) params.append("productId", filters.productId.toString());
    if (filters?.outletId) params.append("outletId", filters.outletId.toString());
    const query = params.toString();
    return fetchApi<{ data: StockAdjustment[]; meta: any }>(`/stock-adjustments${query ? `?${query}` : ""}`);
  },
