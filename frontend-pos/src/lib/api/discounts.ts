...
export const discountsApi = {
  getAll: async (filters?: { tenantId?: string; isActive?: boolean; search?: string }): Promise<{ data: Discount[]; meta: any }> => {
  ...
  getActiveForOutlet: async (tenantId: string, outletId: string): Promise<{ data: Discount[]; meta: any }> => {
    return fetchApi<{ data: Discount[]; meta: any }>(`/discounts/active-for-outlet?tenantId=${tenantId}&outletId=${outletId}`);
  },
