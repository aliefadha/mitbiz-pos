...
export const taxesApi = {
  getAll: async (filters?: { tenantId?: string; isActive?: boolean; search?: string }): Promise<{ data: Tax[]; meta: any }> => {
  ...
  getActiveForOutlet: async (tenantId: string, outletId: string): Promise<{ data: Tax[]; meta: any }> => {
    return fetchApi<{ data: Tax[]; meta: any }>(`/taxes/active-for-outlet?tenantId=${tenantId}&outletId=${outletId}`);
  },
