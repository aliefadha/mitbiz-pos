...
async function main() {
  ...
  console.log('\nStep 2/7: Seeding subscription plans...');
  await seedSubscriptionPlans();

  console.log('\nStep 3/7: Seeding users...');
  const adminId = await seedUser();
  const ownerId = await seedOwnerUser();

  console.log('\nStep 4/7: Seeding tenant...');
