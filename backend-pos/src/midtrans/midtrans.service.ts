...
export class MidtransService {
  ...
  async createSnapTransaction(planId: string, tenantId: string, userId: string) {
    ...
    const parameter = {
      ...
      callbacks: {
        finish: `${this.configService.get<string>('FRONTEND_URL') || 'http://localhost:3000'}/subscription?payment=success`,
      },
      notificationUrl: `${this.configService.get<string>('API_URL') || 'http://localhost:3001'}/api/payments/notification`,
    };
